    ...
    while True:
        ...
        if words[1] == b'000f0010':
            ...
            for i in range(0, len(addr_active)):
                if(addr_active[i] == addr):
                        if(addr_unames[i] != '$none'):
                            conn.send(b'OSSA-PTC: 002ba110\rCount: 3\rContent:next\r{\"username\":\"wlad Bistrecki\"}\rContent:next\r{\"username\":\"Aleksandr Rakutin\"}\rContent:next\r{\"username\":\"Andrej Liapejkow\"}\n')
                            print("MU")
                        else:
                            conn.send(b'OSSA-PTC: 0000f10a\rERROR: 51\n')
                            print("MF")
